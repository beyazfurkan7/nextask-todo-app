    ...
    <div x-data="{ 
        ...
        <template x-teleport="body">
            <div x-show="toastOpen" 
                ...
                <button x-show="deletedTaskId" @click="restoreTask()" class="text-xs font-bold text-indigo-300 hover:text-white transition-colors uppercase tracking-wider px-3 py-1.5 rounded-lg bg-white/10 hover:bg-white/20 focus:ring-2 focus:ring-indigo-400 focus:outline-none">
                    Undo
                </button>
            </div>
        </template>
            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="mb-8 flex flex-col md:flex-row gap-4 items-center justify-between">
                    ...
                    <form action="{{ route('projects.store') }}" method="POST" @submit="$dispatch('form-submitted', 'Creating project...')" class="w-full md:w-auto flex gap-3">
                        @csrf
                        <input type="text" name="title" placeholder="New project name..." class="w-full md:w-64 border-slate-200 bg-white focus:border-indigo-500 focus:ring-indigo-500 rounded-xl shadow-sm text-sm transition-all duration-200" required>
                        <button type="submit" class="bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-2 px-5 rounded-xl shadow-md shadow-indigo-200 transition-all duration-200 focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:outline-none flex-shrink-0">Create</button>
                    </form>
                </div>
                ...
                <div class="mb-8 flex flex-col sm:flex-row gap-4 items-center justify-between bg-white p-3 rounded-2xl shadow-sm border border-slate-200">
                    <div class="flex gap-1 bg-slate-100/50 p-1 rounded-xl w-full sm:w-auto">
                        <a href="{{ route('dashboard', ['filter' => 'all', 'sort' => request('sort', 'manual')]) }}" class="flex-1 text-center sm:flex-none px-4 py-1.5 rounded-lg text-sm font-medium transition-colors {{ request('filter', 'all') === 'all' ? 'bg-white text-indigo-700 shadow-sm border border-slate-200/50' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-200/50' }}">All</a>
                        <a href="{{ route('dashboard', ['filter' => 'active', 'sort' => request('sort', 'manual')]) }}" class="flex-1 text-center sm:flex-none px-4 py-1.5 rounded-lg text-sm font-medium transition-colors {{ request('filter') === 'active' ? 'bg-white text-indigo-700 shadow-sm border border-slate-200/50' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-200/50' }}">Active</a>
                        <a href="{{ route('dashboard', ['filter' => 'completed', 'sort' => request('sort', 'manual')]) }}" class="flex-1 text-center sm:flex-none px-4 py-1.5 rounded-lg text-sm font-medium transition-colors {{ request('filter') === 'completed' ? 'bg-white text-indigo-700 shadow-sm border border-slate-200/50' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-200/50' }}">Completed</a>
                    </div>
                    <div class="flex items-center gap-3 w-full sm:w-auto px-2">
                        <span class="text-sm font-semibold text-slate-400">Sort by:</span>
                        <div class="flex gap-1 bg-slate-100/50 p-1 rounded-xl w-full sm:w-auto">
                            <a href="{{ route('dashboard', ['filter' => request('filter', 'all'), 'sort' => 'manual']) }}" class="flex-1 text-center sm:flex-none px-4 py-1.5 rounded-lg text-sm font-medium transition-colors {{ request('sort', 'manual') === 'manual' ? 'bg-white text-slate-800 shadow-sm border border-slate-200/50' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-200/50' }}">Manual</a>
                            <a href="{{ route('dashboard', ['filter' => request('filter', 'all'), 'sort' => 'date']) }}" class="flex-1 text-center sm:flex-none px-4 py-1.5 rounded-lg text-sm font-medium transition-colors {{ request('sort') === 'date' ? 'bg-white text-slate-800 shadow-sm border border-slate-200/50' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-200/50' }}">Date</a>
                            <a href="{{ route('dashboard', ['filter' => request('filter', 'all'), 'sort' => 'priority']) }}" class="flex-1 text-center sm:flex-none px-4 py-1.5 rounded-lg text-sm font-medium transition-colors {{ request('sort') === 'priority' ? 'bg-white text-slate-800 shadow-sm border border-slate-200/50' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-200/50' }}">Priority</a>
                        </div>
                    </div>
                </div>
                @endif
                        ...
                        <div class="rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex flex-col transition-colors duration-300 relative" 
                            ...
                            <div class="p-5 border-b border-slate-100/50 bg-white/40 backdrop-blur-sm flex justify-between items-center group">
                                ...
                                <form x-show="editProject" action="{{ route('projects.update', $project->id) }}" method="POST" class="flex flex-col gap-3 w-full" style="display: none;">
                                    ...
                                    <div class="flex gap-2 items-center w-full">
                                        <input type="text" name="title" value="{{ $project->title }}" class="flex-1 border-slate-200 bg-white/80 rounded-lg focus:border-indigo-500 focus:ring-indigo-500 text-sm px-3 py-1.5" required>
                            ...
                            <div class="p-4 flex-1">
                                <ul class="space-y-2"
                                    ...
                                    @forelse($project->tasks as $task)
                                        <li class="w-full bg-white/50 rounded-xl" data-id="{{ $task->id }}" 
                                            x-data="{ 
                                                editTask: false, 
                                                deletingTask: false,
                                                deleteTask() {
                                            ...
                                            <div x-show="!editTask" class="flex flex-col sm:flex-row sm:justify-between sm:items-center group p-3 rounded-xl border border-transparent hover:border-slate-200 hover:bg-white transition-all duration-200 gap-2 shadow-sm">
                                                <div class="flex items-start gap-2">
                                                    ...
                                                    <div class="flex flex-col">
                                                        <span class="text-sm font-medium transition-colors duration-200 {{ $task->is_completed ? 'text-slate-400 line-through' : 'text-slate-700' }}">{{ $task->content }}</span>
                                                        <div class="flex flex-wrap items-center gap-2 mt-1.5">
                                                            @if($task->priority === 'high')<span class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-red-100 text-red-700 uppercase tracking-wider">High</span>
                                                            @elseif($task->priority === 'medium')<span class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-amber-100 text-amber-700 uppercase tracking-wider">Medium</span>
                                                            @else<span class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-slate-100 text-slate-600 uppercase tracking-wider">Low</span>@endif
                                                            ...
                                                            @if($task->due_date)
                                                                ...
                                                                @endphp
                                                                <span class="inline-flex items-center gap-1 text-[11px] font-medium {{ $task->is_completed ? 'text-slate-400' : ($isOverdue ? 'text-red-600 font-bold' : ($isToday ? 'text-amber-600 font-bold' : 'text-slate-500')) }}">
                                                ...
                                                <div class="flex items-center self-end sm:self-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200 ml-8 sm:ml-0">
                                                    <button @click="editTask = true" class="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-slate-100 rounded-md transition-colors"><svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg></button>
                                                    <form @submit.prevent="deleteTask()" class="m-0 flex items-center">
                                                        <button type="submit" class="p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-md transition-colors"><svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>
                                                    </form>
                                                </div>
                                            </div>

                                            <form x-show="editTask" action="{{ route('tasks.update', $task->id) }}" method="POST" class="w-full mt-2 p-3 bg-white rounded-xl border border-slate-200 shadow-sm" style="display: none;">
                                                @csrf @method('PUT')
                                                <input type="text" name="content" value="{{ $task->content }}" class="w-full border-slate-300 rounded-lg focus:border-indigo-500 focus:ring-indigo-500 text-sm py-1.5 mb-3" required>
                                                <div class="flex flex-col sm:flex-row gap-3 items-end sm:items-center">
                                                    <div class="flex gap-2 w-full sm:w-auto flex-1">
                                                        <input type="date" name="due_date" value="{{ $task->due_date ? $task->due_date->format('Y-m-d') : '' }}" class="w-1/2 sm:w-auto border-slate-300 rounded-lg focus:border-indigo-500 focus:ring-indigo-500 text-xs py-1.5 text-slate-600">
                            ...
                            <template x-teleport="body">
                                <div x-show="taskModalOpen" style="display: none;" class="relative z-[99999]">
                                    <div x-show="taskModalOpen" class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm transition-opacity"></div>
                                    <div class="fixed inset-0 z-10 overflow-y-auto">
                                        <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                                            <div x-show="taskModalOpen" @click.away="taskModalOpen = false" class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-lg border border-slate-200">
                                                <form action="{{ route('tasks.store', $project->id) }}" method="POST">
                                                    @csrf
                                                    <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                                                        <h3 class="text-lg font-bold leading-6 text-slate-900" id="modal-title">New Task</h3>
                                                        <div class="mt-4"><input type="text" name="content" placeholder="What needs to be done?" class="w-full border-slate-200 rounded-xl focus:border-indigo-500 focus:ring-indigo-500 text-sm p-3 shadow-sm bg-slate-50 focus:bg-white" required autofocus></div>
                                                        <div class="flex gap-4 mb-2 mt-4">
                                                            <div class="w-1/2"><label class="block text-xs font-semibold text-slate-700 mb-1">Due Date</label><input type="date" name="due_date" class="w-full border-slate-200 rounded-xl focus:border-indigo-500 focus:ring-indigo-500 text-sm p-2.5 shadow-sm bg-slate-50 focus:bg-white text-slate-600"></div>
                                                            <div class="w-1/2"><label class="block text-xs font-semibold text-slate-700 mb-1">Priority</label><select name="priority" class="w-full border-slate-200 rounded-xl focus:border-indigo-500 focus:ring-indigo-500 text-sm p-2.5 shadow-sm bg-slate-50 focus:bg-white text-slate-600"><option value="low">Low</option><option value="medium" selected>Medium</option><option value="high">High</option></select></div>
                                                        </div>
                                                    </div>
                            ...
                            <template x-teleport="body">
                                <div x-show="deleteProjectModalOpen" style="display: none;" class="relative z-[99999]">
                                    <div x-show="deleteProjectModalOpen" class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm transition-opacity"></div>
                                    <div class="fixed inset-0 z-10 overflow-y-auto">
                                        <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                                            <div x-show="deleteProjectModalOpen" @click.away="deleteProjectModalOpen = false" class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-lg border border-red-100">
                                                <form action="{{ route('projects.destroy', $project->id) }}" method="POST">
                                                    @csrf @method('DELETE')
                                                    <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                                                        <h3 class="text-lg font-bold leading-6 text-slate-900">Delete Project</h3>
                                                        <div class="mt-2"><p class="text-sm text-slate-500">Are you sure you want to delete the project?</p></div>
                                                    </div>
                                                    <div class="bg-slate-50 px-4 py-4 sm:flex sm:flex-row-reverse sm:px-6 border-t border-slate-200">
